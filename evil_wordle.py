...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for i, char in enumerate(guessed_word):
            ...
            if feedback_colors[i] == NOT_IN_WORD_COLOR and self.colors[char]\
                  != CORRECT_COLOR and self.colors[char] != WRONG_SPOT_COLOR:
    ...
    def __str__(self):
        ...
        for i, _ in enumerate(self.rows):
            ...
            for j, char in enumerate (self.rows[i]):
                color_char= color_word(self.colors[char], self.rows[i][j])
                fs[i]+=f"{color_char} "

            fs[i]=fs[i][:-1]

        formatted_string = f"{fs[0]}\n {fs[1]}\n   {fs[2]}"
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i, char in enumerate(secret_word):
        if guessed_word[i] in secret_word and \
            feedback[i] != CORRECT_COLOR and char_freq[guessed_word[i]] > 0:
            feedback[i] = WRONG_SPOT_COLOR
            char_freq[guessed_word[i]] -= 1
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    # make a dictionary containing keys of color_codes and
    # there corresponding word families/group of possible words
    # initialize word_families_dict
    word_families_dict = {}
    feedback_colors_dict={}
    for feedback_colors in feedback_colors_lst:
        #turn color code list into a string, so it can be used as a dictionary key
        feedback_colors_str=str(feedback_colors)
        #save code list into dictionary using feedback_colors_str as key
        feedback_colors_dict[feedback_colors_str]=feedback_colors
        #initialize an empty list that will store all the words in a word family
        word_families_dict[feedback_colors_str] = []
    #populate word_families_dict
    for _, secret_word in enumerate(remaining_secret_words):
        #generate the feedback colors list and convert to string that will be used as dictionary key
        feedback_colors_str = str(get_feedback_colors(secret_word, guessed_word))

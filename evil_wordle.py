...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        post: None
        """
        # have to use guessed_word
        for i, char in enumerate(guessed_word):
            if feedback_colors[i] == CORRECT_COLOR:
                self.colors[char] = CORRECT_COLOR
            if feedback_colors[i] == WRONG_SPOT_COLOR and self.colors[char] != CORRECT_COLOR:
                self.colors[char] = WRONG_SPOT_COLOR
            if feedback_colors[i] == NOT_IN_WORD_COLOR and self.colors[char] != CORRECT_COLOR and self.colors[char] != WRONG_SPOT_COLOR:
                self.colors[char] = NOT_IN_WORD_COLOR
            if feedback_colors[i] == NO_COLOR and self.colors[char] != CORRECT_COLOR:
                self.colors[char] = NO_COLOR
    ...
    def __str__(self):
        ...
        for i, _ in enumerate(self.rows):
            ...
            for j, char in enumerate (self.rows[i]):
                colorChar= color_word(self.colors[char], self.rows[i][j])
                formatted_strings[i]+=f"{colorChar} "
            
            formatted_strings[i]=formatted_strings[i][:-1]

        formatted_string = f"{formatted_strings[0]}\n {formatted_strings[1]}\n   {formatted_strings[2]}"
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        for i, color in enumerate(self.feedback_colors):
...
def fast_sort(lst):
    ...
    less, equal, more = [], [], []
    for word_family in lst:
        if word_family < pivot:
            less.append(word_family)
        elif pivot < word_family:
            more.append(word_family)
        else:
            equal.append(word_family)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    for i, char in enumerate(secret_word):
        if guessed_word[i] == secret_word[i] and char_freq[guessed_word[i]] > 0:
            feedback[i] = CORRECT_COLOR
            char_freq[guessed_word[i]] -= 1
        else:
            feedback[i] = NOT_IN_WORD_COLOR
   
    for i, char in enumerate(secret_word):
        if guessed_word[i] in secret_word and feedback[i] != CORRECT_COLOR and char_freq[guessed_word[i]] > 0:
                feedback[i] = WRONG_SPOT_COLOR
                char_freq[guessed_word[i]] -= 1
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for _, secret_word in enumerate(remaining_secret_words):
        feedback_colors_lst.append(get_feedback_colors(secret_word, guessed_word))
    # make a dictionary containing keys of color_codes and there corresponding word families/group of possible words
    # initialize word_families_dict
    word_families_dict = {}
    feedback_colors_dict={}
    for feedback_colors in feedback_colors_lst:
        feedback_colors_str=str(feedback_colors) #turn color code list into a string, so it can be used as a dictionary key
        feedback_colors_dict[feedback_colors_str]=feedback_colors #save code list into dictionary using feedback_colors_str as key
        word_families_dict[feedback_colors_str] = [] #initialize an empty list that will store all the words in a word family
    #populate word_families_dict 
    for _, secret_word in enumerate(remaining_secret_words):
        feedback_colors_str = str(get_feedback_colors(secret_word, guessed_word)) #generate the feed back colors list and convert to string that will be used as dictionary key
        word_families_dict[feedback_colors_str] += [secret_word] #build word list
    # Make a word familiy lst
    word_family_list = []
    for feedback_colors_str in word_families_dict:
        word_family_list.append(WordFamily(feedback_colors_dict[feedback_colors_str], word_families_dict[feedback_colors_str]))
